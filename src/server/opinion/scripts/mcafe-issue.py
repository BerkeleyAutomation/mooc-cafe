...
for s in statements:
    ...
    for user in active_users:
        ...
        if len(user_s_rating)>1: #rate more than 1 time, get user log info
            s_log_skip=LogUserEvents.objects.filter(is_visitor=False, logger_id=user_s_rating[0].user.id,log_type=11,details__contains='skip').filter(details__contains='slider_set '+str(s.id)).order_by('-created')
            s_log_rating=LogUserEvents.objects.filter(is_visitor=False, logger_id=user_s_rating[0].user.id,log_type=11).exclude(details__contains='skip').exclude(details__contains='grade').filter(details__contains='slider_set '+str(s.id)).order_by('-created')
            if len(s_log_skip)==0: #no skip
                if len(s_log_rating)>0:
